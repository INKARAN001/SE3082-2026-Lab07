...
int main(int argc, char **argv) {
    ...
    /*
     * ALLREDUCE: same as Reduce but with no root parameter —
     * every process ends up with the global total in total_sum.
     */
    long long total_sum = 0;
    MPI_Allreduce(&local_sum, &total_sum, 1, MPI_LONG_LONG,
                  MPI_SUM, MPI_COMM_WORLD);

    /* Every process can now use total_sum, e.g. to compute its share. */
    double percentage = 100.0 * (double)local_sum / (double)total_sum;
    printf("  Rank %d: indices [%d, %d) local_sum = %lld, total_sum = %lld, "
           "contribution = %.2f%%\n",
           rank, start_idx, end_idx, local_sum, total_sum, percentage);
